...
export const GET=()=>{
  const pages=[['/',snapshot.lastReviewAt],['/zh/',snapshot.lastReviewAt],['/rss/',snapshot.lastReviewAt],['/zh/rss/',snapshot.lastReviewAt],['/api/',snapshot.lastReviewAt],...snapshot.events.flatMap(event=>[[`/resets/${event.id}/`,event.announcedAt],[`/zh/resets/${event.id}/`,event.announcedAt]])];
