...
export function apiDocument(data){
  ...
  return {apiVersion:'1.0',generatedAt:data.lastSuccessAt||data.lastReviewAt||null,source:{account:'@thsottiaux',url:'https://x.com/thsottiaux',collection:data.collectorMethod||'browser_review',coverage:data.coverage||'partial'},status:{lastConfirmedReset:completed[0]||null,pendingAnnouncement:pending,confirmedResetCount:completed.length,averageIntervalSeconds:intervals.length?Math.round(intervals.reduce((sum,value)=>sum+value,0)/intervals.length/1000):null},events};
}
export function openAPI(origin='https://codexresets.net'){
  return {openapi:'3.1.0',info:{title:'Codex Resets API',version:'1.0.0',description:'Public reset announcements collected from Tibo’s public X posts.'},servers:[{url:origin}],paths:{'/api/v1/status':{get:{summary:'Current reset status',responses:{200:{description:'Current status'}}}},'/api/v1/events':{get:{summary:'Reset event archive',responses:{200:{description:'Event list'}}}},'/api/v1/events/{id}':{get:{summary:'One reset event',parameters:[{name:'id',in:'path',required:true,schema:{type:'string',pattern:'^[0-9]{10,25}$'}}],responses:{200:{description:'Event'},404:{description:'Not found'}}}}}};
}
