...
export function apiDocument(data){
  ...
  return {apiVersion:'1.0',generatedAt:data.lastSuccessAt||data.lastReviewAt||null,source:{account:'@thsottiaux',url:'https://x.com/thsottiaux',repliesUrl:'https://x.com/thsottiaux/with_replies',collection:data.collectorMethod||'browser_review',coverage:data.coverage||'partial',historyCatalog:data.catalogSource||null,historyCredit:data.catalogSource?'Historical catalog backfilled from codex-resets.com; original X links are retained.':null},status:{lastConfirmedReset:completed[0]||null,pendingAnnouncement:pending,confirmedResetCount:completed.length,averageIntervalSeconds:intervals.length?Math.round(intervals.reduce((sum,value)=>sum+value,0)/intervals.length/1000):null,longestIntervalSeconds:intervals.length?Math.round(Math.max(...intervals)/1000):null},events,posts};
}
