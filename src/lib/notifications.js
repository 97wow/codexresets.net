...
function emailWords(lang){return ({
  ...
  es:{confirmSubject:'Confirma tus alertas de Codex Resets',confirmTitle:'Confirmar alertas por correo',confirmText:'Confirma tu suscripción. Te avisaremos cuando haya una nueva señal, anuncio o restablecimiento.',confirmAction:'Confirmar alertas',alertSubject:'Nueva actualización de Codex',alertTitle:'Nueva actualización de Codex',source:'Ver publicación en X',unsubscribe:'Cancelar suscripción'},
  fr:{confirmSubject:'Confirmez vos alertes Codex Resets',confirmTitle:'Confirmer les alertes e-mail',confirmText:'Confirmez votre abonnement. Nous vous préviendrons lors d’un nouveau signal, d’une annonce ou d’une réinitialisation.',confirmAction:'Confirmer les alertes',alertSubject:'Nouvelle actualité Codex',alertTitle:'Nouvelle actualité Codex',source:'Voir la publication X',unsubscribe:'Se désabonner'},
  de:{confirmSubject:'Codex Resets Benachrichtigungen bestätigen',confirmTitle:'E-Mail-Benachrichtigungen bestätigen',confirmText:'Bestätige dein Abonnement. Wir informieren dich bei neuen Signalen, Ankündigungen oder Zurücksetzungen.',confirmAction:'Benachrichtigungen bestätigen',alertSubject:'Neues Codex-Update',alertTitle:'Neues Codex-Update',source:'X-Beitrag öffnen',unsubscribe:'Abbestellen'},
  pt:{confirmSubject:'Confirme seus alertas do Codex Resets',confirmTitle:'Confirmar alertas por e-mail',confirmText:'Confirme sua assinatura. Avisaremos quando houver um novo sinal, anúncio ou redefinição.',confirmAction:'Confirmar alertas',alertSubject:'Nova atualização do Codex',alertTitle:'Nova atualização do Codex',source:'Ver publicação no X',unsubscribe:'Cancelar assinatura'},
  ru:{confirmSubject:'Подтвердите уведомления Codex Resets',confirmTitle:'Подтвердите уведомления по почте',confirmText:'Подтвердите подписку. Мы сообщим о новом сигнале, анонсе или сбросе лимитов.',confirmAction:'Подтвердить уведомления',alertSubject:'Новое обновление Codex',alertTitle:'Новое обновление Codex',source:'Открыть публикацию в X',unsubscribe:'Отписаться'},
...
export async function handleTelegramUpdate(env,request){
  ...
  const confirmations={zh:'已开启 Codex 重置提醒。发送 /stop 可随时关闭。','zh-Hant':'已開啟 Codex 重置提醒。傳送 /stop 可隨時關閉。',es:'Las alertas de Codex están activadas. Envía /stop para cancelar.',fr:'Les alertes Codex sont activées. Envoyez /stop pour vous désabonner.',de:'Codex-Benachrichtigungen sind aktiv. Mit /stop kannst du sie abbestellen.',pt:'Os alertas do Codex estão ativos. Envie /stop para cancelar.',ru:'Уведомления Codex включены. Отправьте /stop, чтобы отписаться.',ja:'Codexリセット通知を有効にしました。/stop でいつでも解除できます。',ko:'Codex 재설정 알림을 켰습니다. /stop으로 언제든 해지할 수 있습니다.',en:'Codex reset alerts are on. Send /stop at any time to unsubscribe.'};await env.RESETS.put(`notify:subscriber:telegram:${await digest(String(chatId))}`,JSON.stringify({channel:'telegram',chatId:String(chatId),lang:langOf(pending.lang),createdAt:new Date().toISOString()}));await env.RESETS.delete(pendingKey);await telegramSend(env,chatId,confirmations[pending.lang]||confirmations.en);return {ok:true};
}
...
async function deliver(env,subscriber,event,subscriberKey){
  ...
  }else if(subscriber.channel==='telegram'){const titles={zh:'Codex 重置动态','zh-Hant':'Codex 重置動態',es:'Actualización de Codex',fr:'Actualité Codex',de:'Codex-Update',pt:'Atualização do Codex',ru:'Обновление Codex',ja:'Codexリセット更新',ko:'Codex 재설정 소식',en:'Codex reset update'};await telegramSend(env,subscriber.chatId,`${titles[lang]||titles.en}\n\n${summary}\n\n${source}`);}
}
