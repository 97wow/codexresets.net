...
export function renderTracker(data,lang='en',now=Date.now()){
  ...
  const notificationCenter=`<dialog class="notify-dialog" aria-labelledby="notify-title"><div class="notify-dialog-head"><div><h2 id="notify-title">${t.notificationCenterTitle}</h2><p>${t.notificationCenterIntro}</p></div><button class="notify-close" type="button" aria-label="${t.notificationClose}">×</button></div><div class="notify-channels">
    ...
    <article class="notify-channel rss-channel"><span class="notify-icon" aria-hidden="true">◔</span><div><h3>${t.channelRss}</h3><p>${t.channelRssDesc}</p></div><a class="channel-action" href="${feed}">${t.rssAction}</a></article>
  ...
  return `<section class="status-panel" aria-labelledby="status-title"><div class="status-top"><div><p>${t.headline}</p><h1 id="status-title">${t.statusHeadline}</h1></div><a class="source-profile" href="https://x.com/thsottiaux" target="_blank" rel="noopener noreferrer"><img class="tibo-avatar" src="/tibo.jpg" alt="" width="42" height="42"><span><strong>Tibo <i class="verified" aria-label="Verified">✓</i></strong><span>@thsottiaux ↗</span></span></a></div>
    ...
    <div class="alert-box"><div><strong>${t.getNotified}</strong><span class="notify-note">${t.notifyNote}</span></div><div class="alert-actions"><button class="notify-open" type="button">${t.notifyAction}</button><a href="${feed}">${t.rss} ↗</a></div></div>
